...
const promiseDataTo = (_endpoint, inputData, options = {}) => new Promise((resolve, reject) => {
  ...
  const doRequest = (attempt = 1) => {
    ...
    const request = endpoint.http.request(requestOptions, async (res) => {
      ...
      if (res.statusCode === 400) {
        logr[config.EMHC_LOG_LEVEL]({ responseText: stats.responseText })

        return reject(new ResponseError('400 Status', stats))
      }
  ...
  function tryAgain (error, stats, attempt) {
    ...
    const prefixMessage = `try Again has exceeded max delivery attempts (${attempt}):`
    const responseError = new ResponseError(error, stats, prefixMessage)
